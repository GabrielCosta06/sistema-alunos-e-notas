...
void salvarDados() {
    ...
    for (int i = 0; i < num_usuarios; i++) {
        ...
        fprintf(file, "Nome: %s\nEmail: %s\nSenha: %s\nRA: %d\nTipo: %d\nNumNotas: %d\nNotas:",
                u.nome, u.email, u.senha, u.ra, u.tipo, u.num_notas);
...
void carregarDados() {
    ...
    while (fscanf(file, "Nome: %[^\n]\nEmail: %[^\n]\nSenha: %[^\n]\nRA: %d\nTipo: %d\nNumNotas: %d\nNotas:",
                  ...
                  &usuarios[num_usuarios].ra, (int*)&usuarios[num_usuarios].tipo, &usuarios[num_usuarios].num_notas) == 6) {
...
void listarAlunos(Usuario* usuario) {
    ...
    for (int i = 0; i < num_usuarios; i++) {
        if (usuarios[i].tipo == ALUNO) { // Verifica os alunos
